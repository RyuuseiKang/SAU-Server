...
module.exports = (app) => {
    ...
    router.get('/', async (req, res) => {
        ...
        var sql = "select chat.token, chat.timestamp, chat.contents, user.name, user.major, user.profileImage, user.id as uid from chat left join user ON user.token=chat.user_token where post_token = '" + post_token + "';";
        conn.query(sql, function (err, rows, fields) {
            if (err) {
                console.log(err);
                res.send({
